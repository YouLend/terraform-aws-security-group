...
resource "aws_security_group_rule" "ingress_rules" {
  ...
  security_group_id = local.this_sg_id
  security_group_id = var.create_sg ? (
    var.use_name_prefix ? aws_security_group.this_name_prefix[0].id : aws_security_group.this[0].id
  ...
  protocol  = var.rules[var.ingress_rules[count.index]][2]
    ...
    depends_on = var.create_sg ? (
    var.use_name_prefix ? [aws_security_group.this_name_prefix] : [aws_security_group.this]
  ) : []
